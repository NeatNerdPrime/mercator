...
import { Cell, Geometry, InternalEvent } from "@maxgraph/core";
export function setAnnotationArrow(graph, edge) {
    graph.batchUpdate(() => {
...
export function setAnnotationDirectionalArrow(graph, edge) {
    graph.batchUpdate(() => {
        graph.setCellStyles("dashed", true, [edge]);
        graph.setCellStyles("edgeStyle", "straightEdgeStyle", [edge]);
        graph.setCellStyles("startArrow", "none", [edge]);
...
function setOrthogonalArrow(graph, edge) {
    graph.batchUpdate(() => {
...
export function setMessageFlow(graph, edge) {
    graph.batchUpdate(() => {
...
export function setConversationFlow(graph, edge) {
    graph.batchUpdate(() => {
        edge.style.baseStyleNames = ["bpmnConversationLink"];
    });
}
export function setSequenceFlow(graph, edge) {
    graph.batchUpdate(() => {
...
export function setConditionalFlow(graph, edge) {
    graph.batchUpdate(() => {
...
export function setDefaultFlow(graph, edge) {
    graph.batchUpdate(() => {
...
function findEdgeChild(edge, styleName) {
    return edge.children?.find(child => child.style?.baseStyleNames?.includes(styleName)) ?? null;
}
export function setEventFlow(graph, edge, iconValue) {
    graph.batchUpdate(() => {
...
function getEdgeTerminal(edge, source) {
    return source ? edge?.source : edge?.target;
}
function hasStyle(cell, ...names) {
    const baseNames = cell?.style?.baseStyleNames ?? [];
    return names.some(n => baseNames.includes(n));
}
export function installEdgeRules(graph) {
    graph.addListener(InternalEvent.CELL_CONNECTED, (_sender, evt) => {
