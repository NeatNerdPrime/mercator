...
function deriveStyle(base, overrides, remove = []) {
    const s = cloneUtils.clone(base);
 ...
 */
export function applyGraphStyles(graph) {
    //==================================================================
    ...
    const baseVertex = deriveStyle(defaultVertex, {
        whiteSpace: "wrap",
        overflow: "hidden",
        strokeWidth: 2,
    ...
    const processStyle = deriveStyle(baseVertex, {
        shape: "rectangle",
        fontSize: 10,
        rounded: true,
        horizontal: true,
    ...
    //=============================================================
    // Activities
    const activitiesStyle = deriveStyle(baseVertex, {
        shape: "rectangle",
        strokeWidth: 1,
        fontSize: 10,
        rounded: false,
    ...
    const gatewayStyle = deriveStyle(baseVertex, {
        shape: "rhombus",
    ...
    const stateStyle = deriveStyle(baseVertex, {
        shape: "ellipse",
    ...
    // 4) Base stable (EDGE)
    // =========================
    const baseEdge = cloneUtils.clone(stylesheet.getDefaultEdgeStyle());
    ...
    const annotationStyle = deriveStyle(baseVertex, {
    ...
    const pentagonStyle = deriveStyle(baseVertex, {
        shape: "pentagonShape",
        resizable: false,
        // pas de fond
        fillColor: "none",
    ...
    EdgeMarkerRegistry.add("bpmnSlash", (canvas, _shape, _type, pe, unitX, unitY, size, _source, sw, _filled) => {
    ...
    EdgeMarkerRegistry.add("bpmnMessage", (canvas, shape, _type, pe, unitX, unitY, size, _source, sw, _filled) => {
    ...
    const conversationLinkStyle = {
        ...
        ...{ doubleGap: 4 }, // distance entre les 2 traits
    };
    ...
    // =============================================================================
    EdgeMarkerRegistry.add("bpmnEventCircle", (_canvas, _shape, _type, pe, unitX, unitY, size, _source, _sw, _filled) => {
        const r = size * 0.5;
