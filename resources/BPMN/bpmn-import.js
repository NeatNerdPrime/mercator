...
import { addBPMNAnnotation, addBPMNConnection, addBPMNGateway, addBPMNState, addBPMNTask, setIconCellValue } from "./bpmn-helpers";
...
export function getCurrentBpmnData() {
...
export function parseBPMN(xmlText) {
    ...
    for (const laneSet of Array.from(laneSets)) {
        ...
        for (const lane of Array.from(laneElements)) {
            const laneData = {
                ...
                flowNodeRefs: Array.from(lane.querySelectorAll('flowNodeRef')).map(ref => ref.textContent?.trim()).filter(Boolean)
            };
            lanes.push(laneData);
    ...
    const data = { elements, positions, labelPositions, xmlDoc };
...
const insertVertex = (graph, options) => {
    const v = graph.insertVertex(options);
    v.getStyle = getStyle;
    return v;
};
const insertEdge = (graph, options) => {
...
export function drawDiagram(graph, data) {
    ...
    try {
        ...
        const getParentAndPosition = (flowNodeId, absolutePos) => {
        ...
        const computeEdgeMidpoint = (waypoints) => {
        ...
        const applyLabelPosition = (vertex, elementId, elementPos) => {
            ...
            const rs = graph.getCellStyle(vertex);
            const vlp = (rs?.verticalLabelPosition ?? 'middle');
            const baseSpacing = (rs?.spacing ?? 2);
            const spacingTop = (rs?.spacingTop ?? 0);
            const spacingLeft = (rs?.spacingLeft ?? 0);
            const spacingRight = (rs?.spacingRight ?? 0);
            let offsetX;
            let offsetY;
            if (vlp === 'bottom') {
        ...
        elements.events.forEach((event) => {
            ...
            const vertex = addBPMNState(graph, eventParent, x, y);
        ...
        elements.tasks.forEach((t) => {
            ...
            const vertex = addBPMNTask(graph, taskParent, x, y);
                ...
                if (flow.waypoints && flow.waypoints.length >= 2) {
                    ...
                    if (geometry) {
                        ...
                        if (sourceGeo && targetGeo) {
                            ...
                            geometry.sourcePoint = new Point(firstWaypoint.x - sourceAbsX, firstWaypoint.y - sourceAbsY);
                            geometry.targetPoint = new Point(lastWaypoint.x - targetAbsX, lastWaypoint.y - targetAbsY);
                            // Points intermédiaires (exclure premier et dernier)
                            if (flow.waypoints.length > 2) {
                                geometry.points = flow.waypoints.slice(1, -1).map((wp) => new Point(wp.x - offsetX, wp.y - offsetY));
                            }
                        ...
                        if (edgeLabelPos && flow.waypoints && flow.waypoints.length >= 2) {
                            ...
                            edgeGeom.offset = new Point((edgeLabelPos.x + edgeLabelPos.width / 2) - mid.x, (edgeLabelPos.y + edgeLabelPos.height / 2) - mid.y);
                ...
                if (flow.waypoints && flow.waypoints.length >= 2) {
                    ...
                    if (geometry) {
                        ...
                        if (msgLabelPos && flow.waypoints && flow.waypoints.length >= 2) {
                            const edgeGeom = edge.getGeometry();
                            const mid = computeEdgeMidpoint(flow.waypoints);
                            edgeGeom.offset = new Point((msgLabelPos.x + msgLabelPos.width / 2) - mid.x, (msgLabelPos.y + msgLabelPos.height / 2) - mid.y);
...
export function bindBpmnFileInput(graph, inputId = 'file-input') {
    ...
    input.addEventListener('change', async (e) => {
        ...
        try {
            ...
            const totalElements = data.elements.events.length +
