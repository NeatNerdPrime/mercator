...
export function makeDefaultHandlers() {
    return {
        "delete": ({ graph, cell }) => {
        ...
        "connect": ({ graph, cell, menuEl }) => {
            ...
            const ch = (graph.connectionHandler ?? graph.getPlugin("ConnectionHandler"));
            // ✅ on définit le factoryMethod (c’est LUI qui est utilisé pour créer l’edge)
            const prevFactory = ch.factoryMethod;
            ch.factoryMethod = (source, target, _styleFromPreview) => {
        ...
        "color": ({ graph, cell, menuEl }) => {
            const swatch = menuEl.closest("[data-color]");
