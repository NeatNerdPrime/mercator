...
export function startPlaceVertexFollowMouse(opts) {
    ...
    const prevSuspended = undoManager.__suspended ?? false;
    const suspendUndo = () => {
        undoManager.__suspended = true;
    };
    const restoreUndo = () => {
        undoManager.__suspended = prevSuspended;
    };
    let placing = true;
    ...
    const getParentCell = (c) => (typeof c?.getParent === "function" ? c.getParent() : c?.parent);
     ...
     */
    const toParentLocalPoint = (parent, pt) => {
        if (!parent)
            return pt;
    ...
    const clampInLaneHeader = (parent, localPt) => {
        if (!parent)
            return localPt;
        if (!isLaneCell(parent))
            return localPt;
    ...
    const moveCellTo = (graphPt) => {
        ...
        try {
            ...
            if (newParent && newParent !== oldParent) {
                ...
                if (model.add)
                    model.add(newParent, cell, idx);
                else if (graph.addCell)
                    graph.addCell(cell, newParent, idx);
