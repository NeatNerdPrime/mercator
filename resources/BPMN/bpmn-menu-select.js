...
import { removeBottomCenterBadge, setAdHocMarker, setCompensationMarker, setLoopMarker, setParallelMarker, setSequentialMarker, setSubProcessMarker } from "./bpmn-badge";
...
import { isActivitiesVertex, isConversationVertex, isDataVertex, isGatewayVertex, isLaneVertex, isProcessVertex, isStateVertex, setDatabaseVertex, setDataVertex, setIconCellValue, setInputDataVertex, setOutputDataVertex } from "./bpmn-helpers";
...
export function setIconValue(graph, iconCell, value) {
...
function getSelectedVertex(graph) {
    const sel = (graph.getSelectionCell()) ||
        (graph.getSelectionModel()?.cell) ||
        null;
    if (!sel)
        return null;
    return (isProcessVertex(graph, sel) ||
...
function getSingleSelectedEdge(graph) {
    const edges = graph
        .getSelectionCells()
        .filter(cell => cell.isEdge());
    return edges.length === 1 ? edges[0] : null;
}
function positionMenuBelowVertexMenu(menu, vertexMenu, verticalOffset = -100) {
...
export function setupBpmnMenuSelect(graph) {
    ensureStylesOnce();
    ...
    const list = menu.querySelector(".bpmn-select-list");
    let elements = [];
    let selectedIndex = 0;
    ...
    const highlightSelected = () => {
        const items = Array.from(list.querySelectorAll(".bpmn-select-item"));
        items.forEach((it, i) => {
    ...
    const onFocusIn = (e) => {
        ...
        if (menu.contains(t) || vertexMenu.contains(t))
            return;
        closeMenu();
    };
    ...
    searchBtn.addEventListener("click", (e) => {
        ...
        if (isDataVertex(graph, vertex)) {
            ...
            fetchInformationObjects()
                .then((objects) => openMenu(objects))
                .catch((error) => console.error("[bpmn-menu-select] Impossible de charger les objets du graph", error));
        }
        else if (isProcessVertex(graph, vertex)) {
            // Fetch the objects
            fetchGraphObjects()
