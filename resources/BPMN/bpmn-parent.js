...
export function findGroupUnderMouse(graph, pt) {
    // Cell sous le pointeur
    let under = typeof graph.getCellAt === "function" ? graph.getCellAt(pt.x - 1, pt.y - 1) : null;
...
export function installDropInActivitiesParent(graph) {
    let reparenting = false;
    graph.addListener(InternalEvent.MOVE_CELLS, (_sender, e) => {
        if (reparenting)
            return;
        const evt = e.getProperty?.("event") ?? null;
        const cells = e.getProperty?.("cells") ?? [];
        if (!evt || !cells.length)
            return;
        ...
        const curParent = typeof cells[0].getParent === "function" ? cells[0].getParent() : cells[0].parent;
        if (curParent === target)
            return;
        reparenting = true;
        try {
