...
function perimeterPoint(bpmnType, g, tx, ty) {
...
function getAbsoluteGeometry(graph, cell) {
    ...
    while (p && p !== graph.getDefaultParent?.()) {
...
function getAbsoluteEdgePoints(graph, e) {
    const view = graph.getView?.();
...
function dedupe(points, eps = 0.5) {
    if (!points.length)
        return points;
    const out = [points[0]];
...
export function exportGraphToBpmn(graph) {
    ...
    const diShapes = [];
    const diEdges = [];
    const ensureId = (c, idx, prefix) => (c.getId?.() ?? `${prefix}_${idx}`);
    // Noeuds
    vertices.forEach((v, i) => {
        const type = v.bpmnType;
        ...
        diShapes.push(`<bpmndi:BPMNShape id="Shape_${id}" bpmnElement="${id}">
    ...
    edges.forEach((e, i) => {
        ...
        const s = e.getTerminal?.(true);
        const t = e.getTerminal?.(false);
        if (!s || !t)
            return;
        const sid = s.getId?.() ?? `Node_${vertices.indexOf(s)}`;
        const tid = t.getId?.() ?? `Node_${vertices.indexOf(t)}`;
        xmlFlows.push(`<sequenceFlow id="${id}" sourceRef="${sid}" targetRef="${tid}"${label ? ` name="${escapeXml(label)}"` : ''}/>`);
        ...
        if (!waypoints) {
            ...
            const sType = s.bpmnType;
            const tType = t.bpmnType;
            const geo = e.getGeometry?.();
            const route = (geo?.points ?? []);
            const approachS = route.length ? route[0] : centerOf(tAbs);
            const approachT = route.length ? route[route.length - 1] : centerOf(sAbs);
        ...
        diEdges.push(`<bpmndi:BPMNEdge id="Edge_${id}" bpmnElement="${id}">
