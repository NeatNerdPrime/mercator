...
export function initZOrder(graph) {
 ...
 */
function getAllCellsRecursive(cell, cells = []) {
...
function organizeBpmnZOrder(graph) {
    ...
    const databases = [];
    const edges = [];
    const eventFlowEdges = []; // ← bpmnEventFlow : toujours au-dessus
    const conversations = [];
    const data = [];
    const activities = [];
    ...
    try {
        if (pools.length > 0)
            graph.orderCells(true, pools);
        if (activities.length > 0)
            graph.orderCells(false, activities);
        if (pools.length > 0)
            graph.orderCells(false, pools);
        if (edges.length > 0)
            graph.orderCells(false, edges);
        if (processes.length > 0)
            graph.orderCells(false, processes);
        if (data.length > 0)
            graph.orderCells(false, data);
        if (databases.length > 0)
            graph.orderCells(false, databases);
        if (gateways.length > 0)
            graph.orderCells(false, gateways);
        if (states.length > 0)
            graph.orderCells(false, states);
        if (annotations.length > 0)
            graph.orderCells(false, annotations);
        if (conversations.length > 0)
            graph.orderCells(false, conversations);
        // ← bpmnEventFlow en dernier : toujours au-dessus de tout
        if (eventFlowEdges.length > 0)
            graph.orderCells(false, eventFlowEdges);
    }
    finally {
        model.endUpdate();
    }
...
//==================================
export function setupBringToFrontKeyBinding(graph, container) {
...
export function bringSelectedToFront(graph) {
...
export function bringSelectedToBack(graph) {
    const selectionCells = graph.getSelectionCells();
    if (!selectionCells || selectionCells.length === 0)
        return;
    graph.batchUpdate(() => {
