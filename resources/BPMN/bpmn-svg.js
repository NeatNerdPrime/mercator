...
export function exportGraphToSvg(graph) {
    // 1) Valide la vue
...
class CustomSvgCanvas2D extends SvgCanvas2D {
    constructor(root, styleEnabled, graph) {
...
export async function embedFontInSvg(svg, options) {
    const mime = options.mime ?? "font/ttf";
    const res = await fetch(options.fontUrl);
    if (!res.ok)
        throw new Error(`Impossible de charger la font: ${res.status} ${res.statusText}`);
