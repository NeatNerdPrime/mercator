...
export function initVertexMenuActions(graph, undoManager) {
    const requireSingle = true;
    const menuEl = document.getElementById("vertex-menu");
    // -------------------------------------------------------
    // Helpers
    function getBaseStyleNames(cell) {
        const s = cell?.style;
    ...
    function applyMenuVisibilityForCell(cell) {
        ...
        // défaut: tout visible (
        const actions = new Set(ALL_ACTIONS);
    ...
    const isMenuCell = (c) => !!c && (c.isVertex() || c.isEdge());
    ...
    const ensureMenuInContainer = () => {
        if (menuEl.parentElement !== graph.container)
            graph.container.appendChild(menuEl);
    };
    // Convertit un MouseEvent en coords relatives au container du graph
    const getLocalPointFromMouse = (e) => {
        const rect = graph.container.getBoundingClientRect();
        const x = e.clientX - rect.left;
        const y = e.clientY - rect.top;
        return { x, y };
    };
    const placeMenuAt = (x, y) => {
    ...
    const showForCellFallback = (cell) => {
        const state = graph.view.getState(cell);
        if (!state)
            return;
        applyMenuVisibilityForCell(cell);
        // No menu for icons
        if (cell?.style?.baseStyleNames?.includes("bpmnIcon"))
            return;
    ...
    const showForCellAtMouse = (cell, e) => {
        // No menu for icons
        if (cell?.style?.baseStyleNames?.includes("bpmnIcon"))
            return;
        applyMenuVisibilityForCell(cell);
    ...
    const updateFromSelection = () => {
        ...
        if (requireSingle) {
            if (cells.length !== 1 || !isMenuCell(cells[0]))
                return hide();
            currentCell = cells[0];
    ...
    // Palette de couleur
    const paletteEl = menuEl.querySelector('[data-role="color-palette"]');
    const colorBtn = menuEl.querySelector('[data-action="color"]');
    function setPaletteOpen(open) {
        if (!paletteEl || !colorBtn)
            return;
    ...
    const onMenuClick = (e) => {
        ...
        e.preventDefault();
        e.stopPropagation();
        if (!currentCell || !isMenuCell(currentCell))
            return;
    ...
    const onGraphClick = (_sender, evt) => {
        const cell = evt.getProperty("cell");
        const me = evt.getProperty("event"); // MouseEvent natif
        if (!cell) {
            hide();
            return;
        }
    ...
    const destroy = () => {
        ...
        graph.view.removeListener(onViewChanged);
        graph.removeListener(onGraphClick);
