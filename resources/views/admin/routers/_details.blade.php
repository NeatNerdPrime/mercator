<table class="table table-bordered table-striped table-report" id="{{ $router->getUID() }}">
    ...
    <tr>
        <th width="10%">
            {{ auth()->user()->hasMultiplePerimeters() ? trans('cruds.perimeter.title_short').' / ' : '' }}{{ trans('cruds.router.fields.name') }}
        </th>
        <td width="20%">
                @if (auth()->user()->hasMultiplePerimeters()){{ $router->perimeter->name }} / @endif
            {{ $router->name }}
        </td>
        <th width="10%">
