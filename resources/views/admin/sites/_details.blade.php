...
<table class="table table-bordered table-striped table-report" id="{{ $site->getUID() }}">
    <tbody>
        <tr>
            <th width="10%">
                {{ auth()->user()->hasMultiplePerimeters() ? trans('cruds.perimeter.title_short').' / ' : '' }}{{ trans('cruds.site.fields.name') }}
            </th>
            <td width="20%">
                @if (auth()->user()->hasMultiplePerimeters()){{ $site->perimeter->nom }} / @endif
            @if($withLink)
            @canShow($site)
