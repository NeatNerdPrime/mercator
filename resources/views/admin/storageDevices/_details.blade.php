...
<table class="table table-bordered table-striped table-report" id="{{ $storageDevice->getUID() }}">
    <tbody>
        <tr>
            <th width="10%">
                {{ auth()->user()->hasMultiplePerimeters() ? trans('cruds.perimeter.title_short').' / ' : '' }}{{ trans('cruds.storageDevice.fields.name') }}
            </th>
            <td width="20%">
                @if (auth()->user()->hasMultiplePerimeters()){{ $storageDevice->perimeter->nom }} / @endif
            @if ($withLink)
