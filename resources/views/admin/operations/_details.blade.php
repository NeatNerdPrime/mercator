@props([
    'operation',
    'withLink' => false,
])

<table class="table table-bordered table-striped table-report" id="{{ $operation->getUID() }}">
    <tbody>
        <tr>
            <th width="10%">
                {{ auth()->user()->hasMultiplePerimeters() ? trans('cruds.perimeter.title_short').' / ' : '' }}{{ trans('cruds.operation.fields.name') }}
            </th>
            <td width="20%">
                @if (auth()->user()->hasMultiplePerimeters()){{ $operation->perimeter->name }} / @endif
            @if ($withLink)
            @canShow($operation)
            <a href="{{ route('admin.operations.show', $operation->id) }}">{{ $operation->name }}</a>
            @elsecanShow
            {{ $operation->name }}
            @endcanShow
            @else
            {{ $operation->name }}
            @endif
            </td>
            <th width="10%">
                {{ trans('cruds.operation.fields.type') }}
            </th>
            <td width="20%">
                {{ $operation->type }}
            </td>
            <th width="10%">
                {{ trans('cruds.operation.fields.attributes') }}
            </th>
            <td width="30%">
                @foreach(explode(" ", (string) $operation->attributes) as $attribute)
                    @if(strlen(trim($attribute)) > 0)
                        <span class="badge badge-info">{{ $attribute }}</span>
                    @endif
                @endforeach
            </td>
        </tr>
        <tr>
            <th>
                {{ trans('cruds.operation.fields.description') }}
            </th>
            <td colspan="5">
                {!! $operation->description !!}
            </td>
        </tr>
        @canAccess(App\Models\Process::class)
        <tr>
            <th>
                {{ trans('cruds.operation.fields.process') }}
            </th>
            <td colspan="5">
                @if ($operation->process!=null)
                    @canShow($operation->process)
                        <a href="{{ route('admin.processes.show',$operation->process->id) }}">
                            {{ $operation->process->name ?? '' }}
                        </a>
                    @elsecanShow
                        {{ $operation->process->name ?? '' }}
                    @endcanShow
                @endif
            </td>
        </tr>
        @endcanAccess
        @canAccess(App\Models\Activity::class)
        <tr>
            <th>
                {{ trans('cruds.operation.fields.activities') }}
            </th>
            <td colspan="5">
                @foreach($operation->activities as $activity)
                    @canShow($activity)
                        <a href="{{ route('admin.activities.show', $activity->id) }}">
                            {{ $activity->name }}
                        </a>
                    @elsecanShow
                        {{ $activity->name }}
                    @endcanShow
                    @if (!$loop->last)
                    ,
                    @endif
                @endforeach
            </td>
        </tr>
        @endcanAccess
        @canAccess(App\Models\Actor::class)
        <tr>
            <th>
                {{ trans('cruds.operation.fields.actors') }}
            </th>
            <td colspan="5">
                @foreach($operation->actors as $actor)
                    @canShow($actor)
                        <a href="{{ route('admin.actors.show', $actor->id) }}">
                            {{ $actor->name }}
                        </a>
                    @elsecanShow
                        {{ $actor->name }}
                    @endcanShow
                    @if (!$loop->last)
                    ,
                    @endif
                @endforeach
            </td>
        </tr>
        @endcanAccess
        @canAccess(App\Models\Task::class)
        <tr>
            <th>
                {{ trans('cruds.operation.fields.tasks') }}
            </th>
            <td colspan="5">
                @foreach($operation->tasks as $task)
                    @canShow($task)
                        <a href="{{ route('admin.tasks.show', $task->id) }}">
                            {{ $task->name }}
                        </a>
                    @elsecanShow
                        {{ $task->name }}
                    @endcanShow
                    @if (!$loop->last)
                    ,
                    @endif
                @endforeach
            </td>
        </tr>
        @endcanAccess
        @if($operation->graphs()->count()>0)
        <tr>
            <th>
                <span style="border: 2px solid grey;
                     color: darkred;
                     padding: 6px 14px;
                     border-radius: 6px;
                     display: inline-flex;
                     align-items: center;
                     gap: 8px;
                     font-weight: 600;
                     background: #eff6ff;">
                    <i class="bi bi-diagram-2-fill" style="font-size: 1.3em;"></i>
                    <span style="color: black;">BPMN</span>
                </span>
            </th>
            <td colspan="5" style="vertical-align: middle;">
                @foreach($operation->graphs() as $graph)
                    @canShow($graph)
                        <a href="{{ route('admin.bpmn.show', $graph->id) }}">
                            {{ $graph->name }}
                        </a>
                    @elsecanShow
                        {{ $graph->name }}
                    @endcanShow
                    @if (!$loop->last)
                    ,
                    @endif
                @endforeach
            </td>
        </tr>
        @endif
    </tbody>
</table>

