...
<table class="table table-bordered table-striped table-report" id="{{ $entity->getUID() }}">
    <tbody>
        <tr>
            <th width="10%">
                {{ auth()->user()->hasMultiplePerimeters() ? trans('cruds.perimeter.title_short').' / ' : '' }}{{ trans('cruds.entity.fields.name') }}
            </th>
            <td width="15%">
                @if (auth()->user()->hasMultiplePerimeters()){{ $entity->perimeter->name }} / @endif
            @if ($withLink)
            @canShow($entity)
