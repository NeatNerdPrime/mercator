...
<table class="table table-bordered table-striped table-report" id="{{ $physicalSecurityDevice->getUID() }}">
    ...
    <tr>
        <th width='10%'>
            {{ auth()->user()->hasMultiplePerimeters() ? trans('cruds.perimeter.title_short').' / ' : '' }}{{ trans('cruds.physicalSecurityDevice.fields.name') }}
        </th>
        <td width="20%">
                @if (auth()->user()->hasMultiplePerimeters()){{ $physicalSecurityDevice->perimeter->nom }} / @endif
        @if($withLink)
